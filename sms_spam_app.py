...
def main():
    ...
    if app_mode == "Home":
        ...
        **Navigate using the sidebar to explore different sections!**
        ...
        st.success(f"✅ Sample dataset loaded successfully with {len(df)} messages")
    ...
    # Data Overview Page
        ...
        with col2:
            ...
            with tab1:
                ham_samples = df[df["label"] == 0]["message"].head(3).tolist()
                for i, msg in enumerate(ham_samples, 1):
                    st.write(f"{i}. {msg[:80]}..." if len(msg) > 80 else f"{i}. {msg}")

            with tab2:
                spam_samples = df[df["label"] == 1]["message"].head(3).tolist()
    ...
    # EDA Page
        ...
        with col1:
            ...
            for label in [0, 1]:
                ...
                ax.hist(subset["message_length"], alpha=0.5, label=label_name, bins=15)
    ...
    # Model Training Page
        ...
        # Train-test split
        test_size = st.slider("Test Set Size (%)", 10, 40, 30, 5)
        ...
        if st.button("🚀 Train Logistic Regression Model", use_container_width=True):
            ...
            st.info("Model already trained. Click the button above to retrain.")

            results = st.session_state.results
            st.metric("Current Accuracy", f"{results['accuracy']:.4f}")
        ...
        if "classifier" not in st.session_state:
            ...
            with col1:
                if st.button("📝 Sample Ham Message", use_container_width=True):
                    st.session_state.sample_message = "Hey, are we still meeting for lunch tomorrow at 1 PM? I'll be there!"

            with col2:
                if st.button("⚠️ Sample Spam Message", use_container_width=True):
                    st.session_state.sample_message = "URGENT! You have won a 1 week FREE membership in our £100,000 Prize Jackpot! Txt the word: CLAIM to No: 81010"
